...
const RE_NEWS = [
  { id: 1, title: "Sun Group thành lập công ty con khu đô thị 1.500 ha ở Hưng Yên", img: IMGS.realestate, time: "2 giờ", views: "12.4K", desc: "Tập đoàn Sun Group vừa ra mắt đơn vị thành viên mới chuyên phát triển khu đô thị quy mô lớn tại tỉnh Hưng Yên." },
  { id: 2, title: "TP HCM xây dựng 6 tòa nhà cao tầng tại khu trung tâm quận 1", img: IMGS.realestate2, time: "3 giờ", views: "8.7K", desc: "Chính phủ phê duyệt chủ trương xây dựng 6 tòa nhà cao tầng thay thế các công trình xuống cấp tại khu vực trung tâm." },
  { id: 3, title: "Giá đất nền vùng ven TP HCM tiếp tục tăng mạnh trong quý II", img: IMGS.realestate3, time: "4 giờ", views: "6.2K", desc: "Giá đất nền khu vực Bình Dương, Long An, Đồng Nai ghi nhận mức tăng 15-20% so với cùng kỳ năm ngoái." },
  { id: 4, title: "Hà Nội: Giá căn hộ chung cư tăng bình quân 18% trong 6 tháng đầu năm", img: IMGS.hanoi, time: "5 giờ", views: "4.9K", desc: "Thị trường căn hộ Hà Nội tiếp tục nóng với giá trung bình đạt 55-70 triệu đồng/m² tại các quận nội thành." },
];

const SPORTS_NEWS = [
  { id: 1, title: "Messi không được đưa vào danh sách 26 cầu thủ dự vòng chung kết World Cup 2026", img: IMGS.stadium, desc: "HLV Lionel Scaloni chính thức xác nhận huyền thoại Messi sẽ vắng mặt trong đội hình Argentina tham dự World Cup 2026.", time: "1 giờ", comments: 89 },
  { id: 2, title: "Messi: 'Mỗi thứ thất bại giúp tôi tiếp thêm động lực'", img: IMGS.football2, desc: "Siêu sao người Argentina chia sẻ cảm xúc về quyết định không tham dự World Cup 2026 và kế hoạch giải nghệ.", time: "2 giờ", comments: 212 },
  { id: 3, title: "Haaland lập cú đúp xuất thần, Na Uy vào knock-out World Cup", img: IMGS.football, desc: "Tiền đạo Erling Haaland ghi 2 bàn trong 10 phút cuối giúp Na Uy ngược dòng đánh bại Áo 3-1.", time: "3 giờ", comments: 145 },
  { id: 4, title: "Tuyển Việt Nam tập huấn tại Nhật Bản trước thềm AFF Cup 2026", img: IMGS.vietnam, desc: "HLV Kim Sang-sik triệu tập 28 cầu thủ cho đợt tập trung chuẩn bị cho AFF Cup 2026 diễn ra vào tháng 12 tới.", time: "4 giờ", comments: 67 },
];

const TECH_NEWS = [
  { id: 1, title: "AI tạo sinh đang định hình lại tương lai ngành giáo dục toàn cầu", img: IMGS.tech, time: "2 giờ", views: "15.2K", desc: "Các công cụ AI như ChatGPT, Gemini và Claude đang thay đổi cách học sinh, sinh viên tiếp cận việc học tập." },
  { id: 2, title: "Việt Nam đặt mục tiêu 30% GDP từ kinh tế số vào năm 2030", img: IMGS.tech2, time: "3 giờ", views: "9.8K", desc: "Chính phủ ban hành nghị quyết thúc đẩy chuyển đổi số quốc gia với mục tiêu kinh tế số chiếm 30% GDP vào năm 2030." },
];
...
const STOCKS = [
  { name: "VN-Index", val: "1,312.45", pct: "+1.17%", up: true },
  { name: "HNX-Index", val: "231.18", pct: "+1.07%", up: true },
  { name: "UPCOM", val: "98.67", pct: "-0.34%", up: false },
  { name: "VN30", val: "1,398.72", pct: "0.00%", flat: true },
];
...
function SectionHeader({ title, tabs, activeTab, onTab }: { title: string; tabs?: string[]; activeTab?: string; onTab?: (t: string) => void }) {
  return (
    <div className="flex items-center justify-between border-b-2 pb-2 mb-4" style={{ borderColor: VN_RED }}>
      <h2 className="text-sm font-black uppercase tracking-wider" style={{ color: VN_RED }}>{title}</h2>
      <div className="flex items-center gap-3 text-xs">
        {tabs?.map((tab) => (
          <button key={tab} onClick={() => onTab?.(tab)}
            className={`transition-colors hidden sm:block ${activeTab === tab ? "font-bold border-b" : "text-gray-500 hover:text-gray-800"}`}
            style={activeTab === tab ? { color: VN_RED, borderColor: VN_RED } : {}}>
            {tab}
          </button>
        ))}
        <a href="#" className="flex items-center gap-0.5 hover:underline font-medium" style={{ color: VN_RED }}>
...
function ArticleCard({ article, horizontal = false, large = false }: {
  article: { title: string; img: string; desc?: string; category?: string; time: string; comments?: number; views?: string };
  horizontal?: boolean; large?: boolean;
}) {
  if (horizontal) {
    return (
      <div className="group flex gap-3 cursor-pointer py-3 border-b border-gray-100 last:border-0">
        <img src={article.img} alt={article.title} className="flex-shrink-0 object-cover rounded"
          style={{ width: large ? 128 : 96, height: large ? 82 : 64 }} />
        <div className="flex-1 min-w-0">
          {article.category && <span className="text-[10px] font-bold uppercase tracking-wider" style={{ color: VN_RED }}>{article.category}</span>}
          <h3 className={`font-semibold text-gray-800 leading-tight mt-0.5 line-clamp-3 group-hover:text-[#e2001a] transition-colors ${large ? "text-sm" : "text-xs"}`}>
            {article.title}
          </h3>
          <div className="flex items-center gap-3 mt-1 text-[11px] text-gray-400">
            <span className="flex items-center gap-1"><Clock size={10} /> {article.time} trước</span>
            {article.views && <span className="flex items-center gap-1"><Eye size={10} /> {article.views}</span>}
            {article.comments !== undefined && <span>{article.comments} bình luận</span>}
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="group cursor-pointer">
      <div className="overflow-hidden rounded">
        <img src={article.img} alt={article.title} className="w-full h-44 object-cover group-hover:scale-105 transition-transform duration-500" />
      </div>
      <div className="mt-2">
        {article.category && <span className="text-[10px] font-bold uppercase tracking-wider" style={{ color: VN_RED }}>{article.category}</span>}
        <h3 className="text-sm font-bold text-gray-800 leading-tight mt-0.5 line-clamp-2 group-hover:text-[#e2001a] transition-colors">{article.title}</h3>
        {article.desc && <p className="text-xs text-gray-500 mt-1 line-clamp-2">{article.desc}</p>}
    ...
    <div className="min-h-screen text-gray-900" style={{ background: "#ffffff", fontFamily: "Noto Sans, sans-serif" }}>
        ...
        <div className="max-w-[1200px] mx-auto px-3">
          ...
          {mobileOpen && (
            <div className="sm:hidden grid grid-cols-3 gap-1 pb-3">
              {NAV_CATS.map((cat) => (
                <button key={cat.id} onClick={() => { setActiveNav(cat.id); setMobileOpen(false); }}
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-[1200px] mx-auto px-3 py-2 flex items-center gap-3">
          <span className="flex-shrink-0 text-white text-[10px] font-black px-2 py-0.5 rounded uppercase tracking-wider" style={{ background: VN_RED }}>Tin nhanh</span>
          <p className="flex-1 text-sm text-gray-700 truncate cursor-pointer hover:text-[#e2001a] transition-colors">{BREAKING[tickerIdx]}</p>
          <div className="flex gap-1 flex-shrink-0">
            {BREAKING.map((_, i) => (
              <button key={i} onClick={() => setTickerIdx(i)} className="w-1.5 h-1.5 rounded-full transition-colors"
        ...
        <div className="flex gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="bg-white rounded border border-gray-200 mb-6 overflow-hidden">
              <div className="px-4 py-2.5 flex items-center gap-2" style={{ background: VN_RED }}>
                <span className="text-white font-black text-sm uppercase tracking-wider">Tin nhanh hôm nay</span>
              </div>
              <div className="divide-y divide-gray-100">
                {QUICK_NEWS.map((n) => (
                  <div key={n.id} className="flex items-start gap-3 px-4 py-2.5 hover:bg-gray-50 cursor-pointer group">
                    <span className="flex-shrink-0 text-[11px] font-bold mt-0.5" style={{ color: VN_RED }}>{n.time}</span>
                    <p className="text-sm text-gray-700 leading-snug group-hover:text-[#e2001a] transition-colors">{n.text}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-white rounded border border-gray-200 p-4 mb-6">
              <SectionHeader title="Bất động sản" tabs={["Chính sách", "Dự án", "Tư vấn", "Thị trường"]} activeTab={reTab} onTab={setReTab} />
              <div className="group cursor-pointer mb-3">
                <div className="flex gap-4">
                  <div className="overflow-hidden rounded flex-shrink-0">
                    <img src={RE_NEWS[0].img} alt={RE_NEWS[0].title} className="object-cover group-hover:scale-105 transition-transform duration-500" style={{ width: 200, height: 128 }} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className="text-base font-bold text-gray-800 leading-tight group-hover:text-[#e2001a] transition-colors" style={{ fontFamily: "Merriweather, Georgia, serif" }}>{RE_NEWS[0].title}</h3>
                    <p className="text-xs text-gray-500 mt-1.5 leading-relaxed line-clamp-3">{RE_NEWS[0].desc}</p>
                    <div className="flex items-center gap-3 mt-2 text-[11px] text-gray-400">
                      <span className="flex items-center gap-1"><Clock size={10} /> {RE_NEWS[0].time} trước</span>
                      <span className="flex items-center gap-1"><Eye size={10} /> {RE_NEWS[0].views}</span>
                    </div>
                  </div>
                </div>
              </div>
              <div className="border-t border-gray-100 pt-3 grid grid-cols-1 sm:grid-cols-3 gap-x-6 gap-y-3">
                {RE_NEWS.slice(1).map((item) => <ArticleCard key={item.id} article={item} horizontal />)}
              </div>
            </div>

            {/* THỂ THAO */}
            <div className="bg-white rounded border border-gray-200 p-4 mb-6">
              <SectionHeader title="Thể thao" tabs={["Bóng đá", "World Cup 2026", "Tennis", "V.League"]} activeTab={sportsTab} onTab={setSportsTab} />
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                {SPORTS_NEWS.map((a) => <ArticleCard key={a.id} article={a} />)}
              </div>
            </div>

            {/* KHOA HỌC */}
            <div className="bg-white rounded border border-gray-200 p-4 mb-6">
              <SectionHeader title="Khoa học - Công nghệ" tabs={["AI", "Khởi nghiệp", "Di động", "Máy tính"]} />
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                {TECH_NEWS.map((a) => <ArticleCard key={a.id} article={a} />)}
              </div>
            </div>
          </div>
